...
QQ 的指令面板 API 会自动去掉名称中的 `/`，而呈现和填入方式由 QQ 客户端决定；机器人同时兼容带 `/` 与不带 `/` 的入口。它们只复用现有处理逻辑：菜单走主菜单，蜡笔板与蜡笔进度走蜡笔板模块，礼包查询走礼包查询，每日单抽走抽取模块，雀魂走雀魂档案，帮助走使用说明。同步会查询再创建或更新，不会每次启动重复添加；非本项目或重复的面板一律保留并记录警告，不会自动删除。
...
入口为“莫纳提姆市政终端 → 市政服务 → 雀魂档案”。先发送 `绑定雀魂 玩家昵称`，机器人通过雀魂牌谱屋的公开接口检索并保存稳定的 `amae_player_id`，而不是只保存昵称。群聊与 C2C 使用各自的 QQ 官方身份键，绝不猜测为同一个用户。

支持：`雀魂`、`雀魂档案`、`我的雀魂`、`雀魂四麻`、`雀魂三麻` 与 `解绑雀魂`。档案以 PNG 卡片展示统计、最近 20 场名次及最近 5 场对局；数据来源与限制见 [NOTICE_MAJSOUL.md](NOTICE_MAJSOUL.md)。牌谱屋数据不是雀魂官方实时数据，可能有同步延迟；搜索缓存 1 小时，统计缓存 8 分钟，最近对局缓存 3 分钟，遇到 429 不会重试或绕过 CAP 验证。
