...
发送 `菜单`、`/menu` 或 `/help` 打开“🍹 莫纳提姆市政终端”。群聊需要先 @艾琳娜，C2C 可直接发送。
...
抽取名单取自 [GameKee 嘟嘟脸恶作剧角色图鉴](https://www.gamekee.com/tr/second/127885) 的韩服目录，不混入国际服目录。当前有效数据为 3★ 102 名、2★ 19 名、1★ 12 名，共 133 名使徒和 133 张头像；一星目录中无名称、无图片的 `???` 空占位已排除。十连结果会生成包含头像、名称和星级的图片卡片。

原始清单保存在 `daily_draw_pool.json`，头像位于 `daily_draw_assets/`。需要同步 GameKee 后可运行 `python scripts/sync_gamekee_daily_draw.py`，构建 EXE 时头像会打包进程序，部署目录中的奖池 JSON 仍用于控制启用名单。

### 使徒图鉴

发送 `/图鉴` 可打开“@用户 的圣团”。图鉴包含全部 133 名韩服使徒：未抽到的项目显示为锁定，已抽到的项目显示头像、名称和当前星级，同时显示收集数量与收集率。

每名使徒第一次获得时按奖池原始星级解锁；此后每重复获得一次，当前星级增加一星。例如 3★ 使徒累计抽到两次后显示为 4★。抽取记录、图鉴数量和升星在同一个 SQLite 事务中提交；旧版本已经产生的十连记录会在首次启动时自动补录到图鉴。
